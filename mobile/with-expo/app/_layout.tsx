...
function RootLayout(): React.JSX.Element {
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "Unknown error during account creation";
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      setError("");
      const newBiometricsId = await capsuleClient.verifyEmailBiometricsId(verificationCode);

      await capsuleClient.registerPasskey(email, newBiometricsId, crypto as webcrypto.Crypto);
  ...
  const handleCreateWallet = async () => {
    ...
    try {
      ...
      const [wallet, recoverySecret] = await capsuleClient.createWallet(WalletType.EVM, false);
      ...
      setError(`Wallet creation failed: ${errorMessage}. Please try again or contact support if the issue persists.`);
    } finally {
      setLoading({ isLoading: false, message: "" });
    }
  };
