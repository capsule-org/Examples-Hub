    ...
    <div>
      {internalStep === 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full">
          {Object.values(OAuthMethod).map((method, index) => {
            const Icon = OAuthDetails[method].icon;
            return (
              <Card
                key={OAuthDetails[method].label}
                className={`
            ...
            ${`delay-${(index % 4) + 1}`}
          `}
                onMouseEnter={() => setHoveredOption(method)}
              ...
              transition-smooth
            `}>
                    {OAuthDetails[method].label}
                  </h3>

                  {hoveredOption === method && (
                    <div
                      className="
                absolute inset-0 
